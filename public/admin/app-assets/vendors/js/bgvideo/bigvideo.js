...
(function (factory) {
  ...
  if (typeof define === "function" && define.amd) {
    ...
    define(["jquery", "videojs", "imagesloaded", "jquery-ui"], factory);
  } else {
  ...
  $.BigVideo = function (options) {
    ...
    function updateSize() {
      var containerW =
          settings.container.outerWidth() < $(window).width()
            ? settings.container.outerWidth()
            : $(window).width(),
        containerH =
          settings.container.outerHeight() < $(window).height()
      ...
      if (settings.container.is($("body"))) {
        $("html,body").css(
          "height",
          $(window).height() > $("body").css("height", "auto").height()
            ? "100%"
            : "auto"
        );
      }
      ...
      if (containerAspect < mediaAspect) {
        ...
        if (currMediaType == "video") {
          player.width(containerH * mediaAspect).height(containerH);
        ...
        // wider
        if (currMediaType == "video") {
          player.width(containerW).height(containerW / mediaAspect);
          $(vidEl)
            .css("top", -(containerW / mediaAspect - containerH) / 2)
            .css("left", 0)
            .css("height", containerW / mediaAspect);
    ...
    function initPlayControl() {
      ...
      player.on("timeupdate", function () {
        if (!isSeeking && player.currentTime() / player.duration()) {
          ...
          $("#big-video-control-timer").text(
            minutes + ":" + seconds + "/" + vidDur
          );
        }
      });
    }
    ...
    function showPoster(source) {
      ...
      $("#big-video-image").imagesLoaded(function () {
        mediaAspect =
          $("#big-video-image").width() / $("#big-video-image").height();
        updateSize();
      });
    }
    ...
    BigVideo.init = function () {
      if (!isInitialized) {
        ...
        player = $(
          '<video id="' +
            vidEl.substr(1) +
            '" class="video-js vjs-default-skin" height="1" width="1" preload="auto" data-setup="{}" ' +
            autoPlayString +
        ...
        player.on("loadedmetadata", function (data) {
          if (document.getElementById("big-video-vid_flash_api")) {
            ...
            mediaAspect =
              document
                .getElementById("big-video-vid_flash_api")
                .vjs_getProperty("videoWidth") /
              document
                .getElementById("big-video-vid_flash_api")
                .vjs_getProperty("videoHeight");
          } else {
            // use html5 player to get mediaAspect
            mediaAspect =
              $("#big-video-vid_html5_api").prop("videoWidth") /
              $("#big-video-vid_html5_api").prop("videoHeight");
          }
          updateSize();
